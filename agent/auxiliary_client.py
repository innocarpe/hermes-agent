...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    """Headers needed for chatgpt.com Codex backend requests.

    Match the official Codex CLI header shape closely enough to avoid
    Cloudflare challenge responses on the ChatGPT Codex backend.
    """
    headers = {
...
def _is_auth_error(exc: Exception) -> bool:
    """Detect expired/invalid auth errors that warrant credential refresh.

    Auxiliary clients can outlive short-lived OAuth/session tokens. When Codex,
    Nous, or Anthropic OAuth-backed clients return HTTP 401/token_expired, we
    should refresh credentials and retry the same provider before falling back
    or surfacing a failure.
    """
    status = getattr(exc, "status_code", None)
    if status == 401:
        return True
...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        # ── Auth refresh retry ───────────────────────────────────────
        # Auxiliary clients are cached and can outlive short-lived OAuth/session
        # tokens. Refresh the same provider once on 401/token_expired before
        # trying broader fallback logic.
